...
// Pull a file out of a running per-site container (via docker exec).
// Used to fetch the site repo's CLAUDE.md so we can ship it down to the
// user's local project folder as part of /setup. Returns the file body
// or null if missing.
function readFromContainer(slug, pathInContainer) {
  try {
    const r = require('child_process').spawnSync(
      'docker',
      ['exec', '-u', 'dev', slug, 'cat', pathInContainer],
      { encoding: 'utf8', timeout: 5000 }
    );
    if (r.status === 0 && r.stdout) return r.stdout;
  } catch (e) {
    console.error(`readFromContainer(${slug}, ${pathInContainer}) failed:`, e.message);
  }
  return null;
}
...
${setupCmd.split('\n').map(l => '      ' + l).join('\n')}
  ...
  It writes these files in this folder:
    ...
    CLAUDE.md            — project guide, pulled from the site repo (if present)

  Then check your tools: if mcp__dev-shell__Bash is now available, jump
  straight to STEP 2. If it's not, ask me to restart Claude Code Desktop
  in this folder and re-paste this message; setup is idempotent.
...
app.get('/setup', (req, res) => {
  ...
  // Pull the site repo's CLAUDE.md (if any). We base64-embed it so its
  // contents can include anything — heredoc delimiters, $vars, backticks
  // — without escape hazards.
  const claudeMd = readFromContainer(site, '/home/dev/app/CLAUDE.md');
  const claudeMdBlock = claudeMd
    ? `
# Drop in the project's CLAUDE.md — but never overwrite a user-curated one.
if [[ -e CLAUDE.md ]]; then
  echo "ℹ CLAUDE.md already exists locally — keeping yours (skipping repo copy)."
else
  base64 -d > CLAUDE.md <<'__CLAUDE_MD_B64_EOF__'
${Buffer.from(claudeMd, 'utf8').toString('base64')}
__CLAUDE_MD_B64_EOF__
  echo "✓ CLAUDE.md downloaded from the site repo."
fi
`
    : `
echo "ℹ The site repo has no CLAUDE.md; skipping that step."
`;
...
# Refuse to overwrite the *infra* files if any already exist — keeps repeat
# runs safe. CLAUDE.md is handled separately below since it's user content.
...
echo "  .claude/proxy.js     — proxy → https://${target}"${claudeMd ? `
echo "  CLAUDE.md            — from the site repo"` : ''}
echo
echo "Next:"
echo "  • Claude Code may pick up .mcp.json + .claude/* automatically."
echo "    Check your tools: if mcp__dev-shell__Bash is available, you're done."
echo "  • If not, restart Claude Code Desktop in this folder."
echo "  • On first MCP use, a browser opens for Google sign-in + Authorize."
echo "  • The preview pane (Cmd+Shift+P) shows the live dev site at https://${target}"
`);
});
